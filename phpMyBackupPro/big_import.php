...
';
            ...
            if (!$_GET['delete']) {
            	if (file_exists($_GET["fn"].".zip")) {
                    echo sprintf(
                        '<br>(<a href="%s?fn=%s.zip&dbn=%s">%s%s.zip</a>)</td></tr>',
                        $_SERVER["PHP_SELF"],
                        $_GET["fn"],
                        $_GET['dbn'],
                        BI_RESTART,
                        ...
                        echo sprintf(
                            '<br>(<a href="%s?fn=%s&dbn=%s">%s%s</a>)</td></tr>',
                            $_SERVER["PHP_SELF"], $_GET["fn"], $_GET['dbn'], BI_RESTART, basename($_GET['fn'])
                        );
                    }
            }
            echo "</table>\n";
            ...
            echo sprintf(
                '&dbn=%s&delete=%s&start=%d&foffset=%d&totalqueries=%d&sn=%d";\',500);</script>',
                $_GET['dbn'], $_GET['delete'], $linenumber, $foffset, $totalqueries, $_GET['sn']
            );
            echo "<noscript>\n";
...
if ($error) {
    echo '<tr><td colspan="2"><div class="red_left">' . $error . "</td></tr>";
}
echo "</table></body>\n</html>";
